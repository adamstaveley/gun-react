...
node.use(cors({ origin: `http://localhost:${ports[i].FE}` }))
...
const sendMail = async (email, token) => {
    ...
    await transporter.sendMail({
        to: email,
        subject: 'Magic login',
        html: `<p>Use this magic link to login: <a href="http://localhost:${ports[i].NODE}/auth/login?email=${email}&token=${token}">LOGIN</a><p>`
    })
}
